...
//register_activation_hook(AIOSEOP_PLUGIN_FILE,'aioseop_activate_pl');

if ( ! defined( 'AIOSEOP_PLUGIN_BASENAME' ) )
    define( 'AIOSEOP_PLUGIN_BASENAME', plugin_basename( AIOSEOP_PLUGIN_FILE ) );
...
if ( ! defined( 'AIOSEOP_PLUGIN_URL' ) )
    define( 'AIOSEOP_PLUGIN_URL', plugin_dir_url( AIOSEOP_PLUGIN_FILE ) );
...
add_filter( 'plugin_action_links_' . plugin_basename(AIOSEOP_PLUGIN_FILE) , 'aiosp_add_action_links', 10, 2 );
...
function  aiosp_action_links ( $actions, $plugin_file,  $action_links = array(), $position = 'after' ) { 
  static $plugin;
  if( !isset($plugin) ) {
      $plugin = plugin_basename( AIOSEOP_PLUGIN_FILE );
  }
...
if ( !function_exists( 'aioseop_init_class' ) ) {
	function aioseop_init_class() {
		global $aiosp;
		load_plugin_textdomain( 'all-in-one-seo-pack', false, dirname( plugin_basename( AIOSEOP_PLUGIN_FILE ) ) . '/i18n/' );
...
register_activation_hook( AIOSEOP_PLUGIN_FILE, 'aioseop_install' );
